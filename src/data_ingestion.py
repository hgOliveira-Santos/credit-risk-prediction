...
# ==============================================================================
def ingest_credit_data(url: str = config.GERMAN_CREDIT_ZIP_URL, overwrite: bool = False) -> pd.DataFrame:
    """
    ...
    if os.path.exists(processed_filepath) and not overwrite:
        logger.info(f"Arquivo de dados já existe em '{processed_filepath}'. Pulando a ingestão.")
    ...
    try:
        ...
        # Etapa de Extração
        extract_success = extract_data_file_from_zip(data_filename, raw_data_dir, processed_data_dir)
...
def extract_data_file_from_zip(filename: str, source_folder: str, destination_folder: str, file_extension: str = ".data") -> bool:
    ...
    try:
        ...
        if not file_found:
            logger.warning(f"Nenhum arquivo com extensão '{file_extension}' foi encontrado em '{source_folder}'.")
            return False
...
def load_data_file(data_filepath: str, column_names: List = config.COLUMN_NAMES) -> pd.DataFrame:
